// https://github.com/operate-bsv/op_agent-js/blob/master/lib/crypto/ecies.js
...
const ECIES = {
  ...
  async encrypt(data, pubKey, opts = {}) {
    ...
    const ephemeralPubKey = secp256k1.derivePublicKeyCompressed(
      ephemeralPrivKey
    )

    // Derive ECDH key and sha512 hash
    const sharedKey = secp256k1.mulTweakPublicKeyCompressed(
      pubKey,
      ephemeralPrivKey
    )
    ...
    const hmacKey = await util.importKey(
      keyM,
